...
function renderChannels(channels) {
  ...
  container.innerHTML = Object.entries(groups).map(([g, chs]) => `
    ...
    <div class="channels-grid">
      ${chs.map(c => `
        <div class="channel-card" onclick="playChannel(this,'${c.url.replace(/'/g,"\\'")}','${c.name.replace(/'/g,"\\'")}')">
...
function playChannel(card, url, name) {
  ...
  document.getElementById('player-channel-name').textContent = cleanName(name);
  document.getElementById('player-bar').classList.add('show');

  status.className = 'player-status loading';
  status.textContent = '⏳ Cargando canal...';
  ...
  if (Hls.isSupported()) {
    ...
    hlsInstance.on(Hls.Events.ERROR, (e, data) => {
      if (data.fatal) {
        ...
        status.textContent = '❌ Canal no disponible en este momento';
      }
    });
    ...
    status.className = 'player-status error';
    status.textContent = '❌ Tu navegador no soporta este formato';
  }
}
